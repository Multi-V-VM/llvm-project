...
fn main() {
    check_ext_llvm_project_version();
...
fn check_ext_llvm_project_version() {
    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let version_file = manifest_dir
        .join("..")
        .join("cmake")
        .join("Modules")
        .join("LLVMVersion.cmake");
    println!("cargo:rerun-if-changed={}", version_file.display());

    let version_text = fs::read_to_string(&version_file).unwrap_or_else(|err| {
        panic!(
            "failed to read ext/llvm-project LLVM version file {}: {}",
            version_file.display(),
            err
        )
    });
    let llvm_major = version_text
        .lines()
        .find_map(|line| {
            let line = line.trim();
            line.strip_prefix("set(LLVM_VERSION_MAJOR ")
                .and_then(|rest| rest.strip_suffix(')'))
                .and_then(|value| value.parse::<u64>().ok())
        })
        .unwrap_or_else(|| {
            panic!(
                "failed to parse LLVM_VERSION_MAJOR from {}",
                version_file.display()
            )
        });

    if llvm_major != CRATE_VERSION.major {
        panic!(
            "vendored llvm-sys is for LLVM {} but ext/llvm-project is LLVM {}",
            CRATE_VERSION.major, llvm_major
        );
    }
}
